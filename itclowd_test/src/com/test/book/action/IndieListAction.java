...
public class IndieListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		IndieBookQueryVo qVo = new IndieBookQueryVo();
		qVo.setStartRow((page-1)*limit);
		qVo.setLimit(limit);
		qVo.setCate(cate);
		qVo.setQuery(query);
...
//		pMap.put("query", query);
		...
		int listCount = ilService.getListCount(qVo);
		ArrayList<IndieBookVo> list = ilService.indieGetList(qVo);
